...
@section('content')
            ...
            <div class="table-card" id="resultCard" style="display:none;">
                <div class="table-header">
                    <div class="table-title">
                        <i class="fas fa-seedling"></i> Hasil Data LM34
                    </div>
                    <div style="display:flex; align-items:center; gap:8px;">
                        ...
                        <button id="btnExportPdf" onclick="exportPdf()"
                            style="
                                                                                            display:inline-flex; align-items:center; gap:5px;
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function get_data_lm34(komoditas, region, plant, tahun, bulan) {
                ...
                fetch(`{{ route('get_data_lm34') }}?${params}`)
                    .then(res => res.json())
                    .then(data => {
                        ...
                        const isNumericCol = h => NUMERIC_COLS.has(h);   // format & align di data row
                        const isSubtotalCol = h => SUBTOTAL_COLS.has(h);  // akumulasi subtotal/total

                        // ── Helper format nilai (column-aware) ────────────────────────
                        const fmt = (v, colName = '') => {
                            // ID / kode → tampil apa adanya (tidak diformat)
                            if (colName && isIdCol(colName)) {
                                return (v === null || v === undefined || v === '') ? '' : String(v);
                            }
                            // Null / kosong
                            if (v === null || v === '' || v === undefined) {
                                return (colName && isHargaCol(colName)) ? '0' : '-';
                            }
                            // Bukan angka bersih → tampil string
                        ...
                        const judulLaporan = `LM34 - Laporan Realisasi Penjualan - ${regionalText} - ${plantText} - Komoditas: ${komoText}  ${bulanText}  ${tahunText}`;

                        // ── Deteksi kolom prodheir2_id (case-insensitive) ─────────────
                        const HEIR2_KEY = headers.find(h =>
                            h.toLowerCase() === 'prodheir2_id' ||
                            h.toLowerCase() === 'prodheir2' ||
                            h.toLowerCase() === 'heir2_id'
                        ) || null;
                        ...
                        // Akumulator subtotal (hanya SUBTOTAL_COLS = volume + nilai)
                        ...
                        // Cari index kolom non-numerik pertama untuk label subtotal (colspan)
                        const labelColIdx = headers.findIndex(h => !isNumericCol(h));

                        // ── Helper: baris subtotal / grand-total ─────────────────────
                        // Hitung berapa kolom non-numerik di awal (leading) → dijadikan colspan label
                        let leadingNonNum = 0;
                        for (let j = 0; j < headers.length; j++) {
                            if (!isNumericCol(headers[j])) leadingNonNum++;  // pakai isNumericCol bukan isSubtotalCol
                            else break;
                        }
                        ...
                        const makeSubtotalRow = (label, acc, bgColor, fontWeight, borderTop, textColor = '#14532d') => {
                            ...
                            headers.forEach((h, idx) => {
                                if (idx === 0) {
                                    // Label cell dengan colspan mencakup semua kolom non-numerik leading
                                    ...
                                    // Sudah dicakup colspan — skip
                                    return;
                                } else if (isSubtotalCol(h)) {
                                    // Kolom numerik → tampilkan subtotal
                                    r += `<td style="padding:6px 14px; border:1px solid #bbf7d0; text-align:right; white-space:nowrap; color:${textColor};">${fmt(acc[h])}</td>`;
                                } else {
                                    // Kolom non-numerik di TENGAH/AKHIR (misal harga yg semua null → bukan angka)
                                    // → tetap render td kosong agar jumlah kolom tidak geser
                        ...
                        let accHeir2 = initAcc(), keyHeir2 = '';
                        let accTotal = initAcc();
                        let rowIdx = 0;

                        rows.forEach((row) => {
                            const heir2Val = HEIR2_KEY ? (row[HEIR2_KEY] ?? '') : '';

                            // Deteksi pergantian grup prodheir2_id
                            if (keyHeir2 !== '' && heir2Val !== keyHeir2) {
                                html += makeSubtotalRow(
                                    `Subtotal  ${keyHeir2}`,
                                    accHeir2, '#f0fdf4', '700', '2px solid #bbf7d0'
                                );
                                accHeir2 = initAcc();
                            }

                            keyHeir2 = heir2Val;
                            addToAcc(accHeir2, row);
                        ...
                        // Subtotal grup terakhir yang belum di-flush
                        if (keyHeir2 !== '') {
                            html += makeSubtotalRow(
                                `Subtotal  ${keyHeir2}`,
                                accHeir2, '#f0fdf4', '700', '2px solid #bbf7d0'
                        ...
                        html += makeSubtotalRow(
                            'JUMLAH TOTAL',
                            accTotal, '#14532d', '900', '3px solid #052e16', '#fff'
                        );
            ...
            document.getElementById('filterForm').addEventListener('submit', function (e) {
                ...
                updateLabels();
                get_data_lm34(komoditas, regional, plant, tahun, bulan);
            });
    ...
    <script>
        ...
        async function exportExcel() {
            ...
            const { rows, headers, judul, numericCols, subtotalCols, heir2Key } = _exportData;

            // Column classifiers (mirror render logic)
            const isIdCol    = h => /(_id$|^id_|_id_|material|prodheir|kdbe|norek|^kode|^no$|^plant|^regional|_desc$|uraian|^nama)/i.test(h);
            const isHargaCol = h => /harga|price|rate|satuan/i.test(h);
            ...
            // Colspan for label in subtotal rows
            let leadingNonNum = 0;
            for (let j = 0; j < headers.length; j++) {
            ...
            ws.columns = headers.map(h => ({
                key: h,
                width: isIdCol(h) ? 14
                    : h.toLowerCase().includes('desc') ? 28
                    : isSubtotalCol(h) ? 18 : 15,
            }));

            const border = (s = 'thin') => ({ top:{style:s}, left:{style:s}, bottom:{style:s}, right:{style:s} });
            ...
            tc.fill = fill('FFFFFFFF'); tc.border = border();
            ...
            // Helper: subtotal/grand-total row
            const addSubRow = (label, acc, bgArgb, textArgb = 'FF14532D', bStyle = 'thin') => {
                const rowData = headers.map((h, idx) => {
                    if (idx === 0) return label;
                    if (idx < labelSpan) return null;
                    if (isSubtotalCol(h)) return parseFloat(acc[h]) || 0;
                    return null;
                });
                ...
                r.eachCell({ includeEmpty:true }, (cell, colNum) => {
                    ...
                    cell.font = { bold:true, italic:true, size:9, color:{argb:textArgb} };
                    cell.border = border(bStyle);
                    if (colNum - 1 >= labelSpan) {
                        if (isSubtotalCol(h)) { cell.numFmt = '#,##0'; cell.alignment = {horizontal:'right'}; }
                        else if (isNumericCol(h)) { cell.alignment = {horizontal:'right'}; }
                    }
                });
                try { ws.mergeCells(rn, 1, rn, labelSpan); } catch(e) {}
                ws.getCell(rn, 1).alignment = { horizontal:'right', vertical:'middle', italic:true };
            };
            ...
            let accHeir2 = initAcc(), keyHeir2 = '', accTotal = initAcc(), rowIdx = 0;

            for (const row of rows) {
                const heir2Val = heir2Key ? (row[heir2Key] ?? '') : '';
                if (keyHeir2 !== '' && heir2Val !== keyHeir2) {
                    addSubRow(`Subtotal  ${keyHeir2}`, accHeir2, 'FFF0FDF4');
                    accHeir2 = initAcc();
                }
                keyHeir2 = heir2Val;
                addToAcc(accHeir2, row);
                ...
                exRow.eachCell({ includeEmpty:true }, (cell, colNum) => {
                    const h = headers[colNum - 1];
                    cell.fill = fill(bg); cell.font = {size:9}; cell.border = border();
                    if (isNumericCol(h)) {
            ...
            if (keyHeir2 !== '') addSubRow(`Subtotal  ${keyHeir2}`, accHeir2, 'FFF0FDF4');
            addSubRow('JUMLAH TOTAL', accTotal, 'FF14532D', 'FFFFFFFF', 'medium');
        ...
        function exportPdf() {
            ...
            const fmtNum = (v, colName = '') => {
                ...
                if (v === null || v === '' || v === undefined) return isHargaCol(colName) ? '0' : '-';
                if (!isStrictNum(v)) return String(v);
                const n = parseFloat(v);
                const hasDec = String(v).includes('.');
                return n.toLocaleString('id-ID', { minimumFractionDigits: hasDec ? 2 : 0, maximumFractionDigits: 2 });
            };
            ...
            const makeSubRow = (label, acc, fillRgb, textRgb = [20, 83, 45]) => {
                const cells = headers.map((h, idx) => {
                    let val = '', halign = 'left';
                    if (idx === 0) { val = label; halign = 'right'; }
                    else if (idx < labelSpan) { val = ''; }
                    else if (isSubtotalCol(h)) { val = fmtNum(acc[h], h); halign = 'right'; }
                    else if (isNumericCol(h))  { val = ''; halign = 'right'; }
                    return { content: val, styles: { halign, fontStyle:'bold', fillColor:fillRgb, textColor:textRgb, fontSize:6.5 } };
                });
                body.push(cells);
            };

            let accHeir2 = initAcc(), keyHeir2 = '', accTotal = initAcc();

            for (const row of rows) {
                const heir2Val = heir2Key ? (row[heir2Key] ?? '') : '';
                if (keyHeir2 !== '' && heir2Val !== keyHeir2) {
                    makeSubRow(`Subtotal  ${keyHeir2}`, accHeir2, [240, 253, 244]);
                    accHeir2 = initAcc();
                }
                keyHeir2 = heir2Val;
                addToAcc(accHeir2, row);
            ...
            if (keyHeir2 !== '') makeSubRow(`Subtotal  ${keyHeir2}`, accHeir2, [240, 253, 244]);
            makeSubRow('JUMLAH TOTAL', accTotal, [20, 83, 45], [255, 255, 255]);
