...
@section('content')
    <div class="lm62-container main-content">
        ...
        <header class="lm-page-header">
            ...
            <div class="lm-header-center">
                ...
                <h1>LM 62 &mdash; Laporan Persediaan Produksi</h1>
            </div>
            ...
            <div class="table-card" id="resultCard" style="display:none;">
                <div class="table-header">
                    <div class="table-title">
                        <i class="fas fa-seedling"></i> Hasil Data LM62
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ── Fetch + Render data dari route get_data_lm62 ─────────────────
            function get_data_lm62(komoditi, region, plant, tahun, bulan) {
                ...
                const params = new URLSearchParams({ komoditi, region, plant, tahun, bulan });
                fetch(`{{ route('get_data_lm62') }}?${params}`)
                    .then(res => res.json())
                    .then(data => {
            ...
            document.getElementById('filterForm').addEventListener('submit', function (e) {
                ...
                updateLabels();
                get_data_lm62(komoditas, regional, plant, tahun, bulan);
            });
    ...
    <script>
        ...
        function getJudulLaporan() {
            ...
            return th ? th.textContent.trim() : 'LM62';
        }
        ...
        async function exportExcel() {
            ...
            const ws = workbook.addWorksheet('LM62');
            ...
            let rowIdx = 0;
            for (const [typeDesc, typeData] of Object.entries(groupedData)) {
                addSubRow(`${typeDesc}`, typeData.acc, 'FFE0F2FE');
                for (const [mat, matData] of Object.entries(typeData.materials)) {
                    addSubRow(`${mat}`, matData.acc, 'FFF0FDF4');
                    matData.rows.forEach(row => {
                        const rowData = headers.map(h => {
                            const v = row[h];
                            return (numericColsExport.has(h) && isNum(v)) ? parseFloat(v) : (v ?? '');
                        });
                        ...
                        exRow.eachCell({ includeEmpty: true }, (cell, colNum) => {
                            ...
                            cell.protection = { locked: false };
                            const h = headers[colNum - 1];
                            if (h && numericColsExport.has(h) && isNum(row[h])) {
                                cell.numFmt = '#,##0';
                                cell.alignment = { horizontal: 'right' };
                            } else {
        ...
        function exportPdf() {
            ...
            for (const [typeDesc, typeData] of Object.entries(groupedData)) {
                makeSubRow(`${typeDesc}`, typeData.acc, [224, 242, 254]); // #e0f2fe
                for (const [mat, matData] of Object.entries(typeData.materials)) {
                    makeSubRow(`${mat}`, matData.acc, [240, 253, 244]); // #f0fdf4
                    matData.rows.forEach(row => {
                        const cells = headers.map((h, idx) => {
                            const v = row[h];
                            const num = numericCols.has(h) && isNum(v);
                            const display = num ? fmtNum(v) : (v ?? '');
                            return {
                                content: display,
                                styles: {
                                    halign: num && !h.toLowerCase().includes('kode') ? 'right' : 'left',
                                    fontSize: 6.5,
                                },
                            };
                        });
