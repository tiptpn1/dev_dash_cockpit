CREATE OR REPLACE PROCEDURE `dashboard-cockpit.data_dash.sp_laporan_lm14_teh_rev`(p_region STRING, p_plant STRING, p_tahun STRING, p_bulan STRING)
BEGIN
...
SET luas_areal_tahun_lalu = (SELECT SUM(ha_planted) from `data_dash.cds_lm14_mf` WHERE status='HM' AND tahun_periode=v_end_tl 
   AND (p_region IS NULL OR p_region = '' 
     OR region=p_region)
   AND (p_plant IS NULL OR p_plant = '' 
     OR plant=p_plant));
SET luas_areal_tahun_ini = (SELECT SUM(ha_planted) from `data_dash.cds_lm14_mf` WHERE status='HM' AND tahun_periode=v_end 
   AND (p_region IS NULL OR p_region = '' 
     OR region=p_region)
   AND (p_plant IS NULL OR p_plant = '' 
     OR plant=p_plant));
...
b_agg AS (
  SELECT
    ...
    SUM(amount)   AS amount
  FROM `data_dash.cds_lm14th_bb`
  WHERE tahun_periode BETWEEN v_start_tl AND v_end           -- ← filter periode
  AND (p_region IS NULL OR p_region = '' OR region = p_region) -- ← filter region
  AND (p_plant IS NULL OR p_plant = '' 
...
sum(if(B.tahun_periode between v_start and v_end, coalesce(B.amount,0), 0)) as nilai_tahun_ini  
 from `data_dash.ref_format_report_rev` as A 
 left join `data_dash.cds_lm14th` as B
on A.jobcode=B.jobcode 
 and A.gl=B.account
 and (p_region IS NULL OR p_region = '' 
 ...
 where A.jenis_laporan='LM14_Teh' 
 and A.jobcode='9901' 
 and A.kode like 'FA%'
...
and A.gl='-'
group by A.uraian) as Y

on X.uraian=Y.uraian
...
SELECT
  ...
  CASE 
    WHEN f.kode='FB01' THEN round((total_9901_tahun_lalu - p.pengurang_tahun_lalu)*100/luas_areal_tahun_lalu,2)
    ELSE round(f.biaya_total_tahun_lalu/luas_areal_tahun_lalu*100,2)
  ...
  CASE 
    WHEN f.kode='FB01' THEN round((total_9901_tahun_ini - p.pengurang_tahun_ini)*100/luas_areal_tahun_ini,2)
    ELSE round(f.biaya_total_tahun_ini/luas_areal_tahun_ini*100,2)
