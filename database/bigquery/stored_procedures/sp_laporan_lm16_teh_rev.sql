CREATE OR REPLACE PROCEDURE `dashboard-cockpit.data_dash.sp_laporan_lm16_teh_rev`(p_region STRING, p_plant STRING, p_tahun STRING, p_bulan STRING)
BEGIN
...
coalesce(SUM(Y.tahun_lalu),0) as tahun_lalu, 
coalesce(SUM(Y.tahun_ini),0) as tahun_ini
FROM `data_dash.ref_format_report_rev` X
LEFT JOIN (
  SELECT
    concat(right(costcenter,2),'-',stppk) as stasiun,
    grpaccttext,
