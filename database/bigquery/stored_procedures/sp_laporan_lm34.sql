CREATE OR REPLACE PROCEDURE `dashboard-cockpit.data_dash.sp_laporan_lm34`(p_komoditi STRING, p_region STRING, p_plant STRING, p_tahun STRING, p_bulan STRING)
BEGIN
...
FROM (
  ...
  GROUP BY plant, region, komoditi, prodheir2_id, material_id, material_desc
) X

WHERE (p_region IS NULL OR p_region = '' OR region=p_region)
  AND (p_plant IS NULL OR p_plant = '' OR plant=p_plant)
GROUP BY prodheir2_id, material_id, material_desc
ORDER BY prodheir2_id, material_id;
