CREATE OR REPLACE PROCEDURE `dashboard-cockpit.data_dash.sp_laporan_lm16_teh`(p_region STRING, p_plant STRING, p_tahun STRING, p_bulan STRING)
BEGIN
...
SELECT X.kode, X.grup, X.uraian, coalesce(Y.tahun_lalu,0) as tahun_lalu, coalesce(Y.tahun_ini,0) as tahun_ini
...
SELECT
  ...
  CASE WHEN LEFT(kode,1) NOT IN ('C','D') THEN ROUND((tahun_lalu*100)/prd_tahun_lalu,2) ELSE NULL END AS biaya_per_kg_tahun_lalu,
  CASE WHEN LEFT(kode,1) NOT IN ('C','D') THEN ROUND((tahun_ini*100)/prd_tahun_ini,2) ELSE NULL END AS biaya_per_kg_tahun_ini
FROM temp_gabungan
order by kode;
