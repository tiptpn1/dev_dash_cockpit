CREATE OR REPLACE PROCEDURE `dashboard-cockpit.data_dash.sp_laporan_lm16_karet_rev`(p_region STRING, p_plant STRING, p_tahun STRING, p_bulan STRING)
BEGIN
...
LEFT JOIN (
  SELECT
    concat(right(costcenter,1),'-',stppk) as stasiun, grpaccttext,
  ...
  WHERE LEFT(profitcenter, 4) IN (SELECT plant FROM plant_list)  
  GROUP BY concat(right(costcenter,1),'-',stppk), grpaccttext
...
-- SELECT lev2, lev1, stasiun, kode, uraian, COUNT(*) as cnt
-- FROM temp_biaya  -- jalankan setelah SP dieksekusi
-- GROUP BY lev2, lev1, stasiun, kode, uraian
-- HAVING cnt > 1;
...
SELECT lev2, lev1, stasiun, kode, grup, uraian, tahun_lalu, tahun_ini
FROM temp_biaya
UNION ALL
SELECT lev2, lev1, stasiun, kode, grup, uraian, tahun_lalu, tahun_ini
FROM temp_produksi;
